...
files = [f for f in os.listdir(path_annotations) if re.match(r'\d\d\d\d-\d\d-\d\d-\d\d-\d\d-\d\d-flying\.txt', f)]
files = [(path_images+f.replace('-flying.txt', '.png'), path_annotations+f) for f in files]
...
# ---------------------------------------------------------------------------------------------------------- #
# Description:                                                                                               #
#         Loads RGB satellite images and airplane annotations given and crops 51x51 positive and negative    #
...
# ---------------------------------------------------------------------------------------------------------- #
# Description:                                                                                               #
#         Loads RGB satellite images and airplane annotations and crops 51x51 negative samples around false  #
...
def update_data(model):
	...
	for img_name, ann_name in files:
		...
		for c, score in dets:
			flag = True
			for cc in nms:
				if np.sqrt((c[0]-cc[0])**2 + (c[1]-cc[1])**2) <= radius:
					flag = False
					break
			if flag:
				nms.append(c)
...
train(model, optimizer, train_imgs, train_labels, 256, 2000)
...
# save model
torch.save(model.state_dict(), './models/flying.pytorch')
